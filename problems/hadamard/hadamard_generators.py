...
def singleStateFunc(initialState, targetState, epsilon):
    ...
    return (testStateParams, testStateParams)

def singleGateFunc(targetGate, epsilon):
    def testGateParams(a1, a2):
        R_z = (qt.sigmaz() * 1j * a1).expm()
        R_y = (qt.sigmay() * 1j * a2).expm()
        propagator = (-1j) * R_z * R_y
        return calcGateFidelity(propagator, targetGate)
    return (testGateParams, testGateParams)
