...
tp = np.random.uniform(low=0, high=3.14, size=(6))

QC = QubitCircuit(3)
QC.add_gate("RX", targets=0, arg_value= tp[0])
QC.add_gate("RX", targets=1, arg_value= tp[1])
QC.add_gate("RX", targets=2, arg_value= tp[2])
QC.add_gate("CNOT", targets = 1, controls = 0)
QC.add_gate("CNOT", targets = 2, controls = 0)
QC.add_gate("RX", targets = 0, arg_value = tp[3])
QC.add_gate("RX", targets = 1, arg_value= tp[4])
QC.add_gate("RX", targets = 1, arg_value= tp[5])
U_list = QC.propagators()

TargetGate = gate_sequence_product(U_list)

class threeQubitCircuitRandom(problem):
